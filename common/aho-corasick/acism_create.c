...
acism_create(MEMREF const* strv, int nstrs)
{
    ...
    ACISM *psp = (ACISM *)calloc(1, sizeof*psp);

    fill_symv(psp, strv, nstrs);
    TNODE *troot = (TNODE *)calloc(psp->nchars + 1, sizeof*troot);
...
interleave(TNODE *troot, int nnodes, int nsyms, TNODE **v1, TNODE **v2)
{
    ...
    char *usev = (char *)calloc(usev_size, sizeof*usev);
            ...
            *startp += (pos - *startp) / nkids;
            ...
            if (last_trans < last) {
                ...
                if (last + nsyms >= usev_size) {
                    usev = (char *)realloc(usev, usev_size << 1);
...
{
    STRASH *sv = (STRASH *)malloc(psp->hash_mod * sizeof*sv), *sp = sv;
    int i;
