...
/// Takes a register containing a search token and a base address, and searches the base address for the search token.
 ...
 *
 * This performs a simple sparser search given the query and input buffer. It only searches for one occurance of the query
 * string in each record. Records are assumed to be delimited by newline.
 *
 ...
 * */
sparser_stats_t *
sparser_search_single(char *input,
        long length,
        const char *query,
        long qlength,
    ...
    for (long i = 0; i < length - VECSZ - shifts; i += VECSZ) {
        ...
        // Found something! Seek back to the beginning of the token and invoke the callback..
        if (matches > 0) {
    ...
    if (stats.sparser_passed > 0) {
        stats.fraction_passed_correct = (double)stats.callback_passed / (double)stats.sparser_passed;
...
Fraction False Positives: %f",
            ...
            stats->sparser_passed,
            stats->callback_passed,
            stats->bytes_seeked_forward,
            stats->bytes_seeked_backward,
